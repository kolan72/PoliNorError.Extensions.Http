...
namespace PoliNorError.Extensions.Http.Tests
{
	...
	public class FallbackAsyncFuncTests
	{
		...
		{
			// Arrange
			static async Task<HttpResponseMessage> expectedFunc(CancellationToken _)
			{
				await Task.Yield();
			...
			// Assert
			Assert.That(fallback.AsyncFunc, Is.EqualTo(expectedFunc));
		}
		...
		public void Should_Allow_AsyncFunc_Property_To_Be_Modified()
		{
			...
			// Assert
			Assert.That(fallback.AsyncFunc, Is.EqualTo(newFunc));
			Assert.That(fallback.AsyncFunc, Is.Not.EqualTo(originalFunc));
		}
		...
		public async Task Should_Return_Correct_Response_From_AsyncFunc()
		{
			...
			static async Task<HttpResponseMessage> TestFunc(CancellationToken _)
			{
				await Task.Yield();
		...
		{
			// Arrange
			static async Task<HttpResponseMessage> TestFunc(CancellationToken ct)
			{
